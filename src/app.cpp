...
namespace Theater
{
	...
	LRESULT App::OnMessage(UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_TIMER:
		{
			...
			const auto elapsedMs = std::chrono::duration_cast<std::chrono::milliseconds>(currentTime - this->timerStart).count();
			if (elapsedMs >= 500)
			{
	...
	bool App::MessageWindowCreate()
	{
		...
		this->messageWindow = ::CreateWindowExW(0, APP_WINDOWCLASS_NAME, APP_WINDOW_NAME, 0, 0, 0, 0, 0, HWND_MESSAGE, nullptr, hInstance, NULL);
	...
	void App::OnWinEvent(HWINEVENTHOOK hWinEventHook, DWORD event, HWND hwnd, LONG idObject, LONG idChild, DWORD idEventThread, DWORD dwmsEventTime)
	{
	...
	void App::WinEventHookProc(HWINEVENTHOOK hWinEventHook, DWORD event, HWND hwnd, LONG idObject, LONG idChild, DWORD idEventThread, DWORD dwmsEventTime)
	{
	...
	bool App::HookRegister()
	{
		...
		this->winEventHook = ::SetWinEventHook(EVENT_SYSTEM_FOREGROUND, EVENT_SYSTEM_FOREGROUND, nullptr, App::WinEventHookProc, 0, 0, WINEVENT_OUTOFCONTEXT | WINEVENT_SKIPOWNPROCESS);
